...
# NOTE: Distributed initialization is now performed inside the `main()`
# function so that this module can be imported in a single-process context
# (e.g. when running `run_profiler.py`) without requiring environment
# variables like WORLD_SIZE / RANK / MASTER_ADDR to be set.
...
# run with
# torchrun --nproc_per_node=1 train_orion.py
...
def main():
    ...
    # -------------------------------------------------
    # Optional distributed initialization (torchrun)
    # -------------------------------------------------
    if "WORLD_SIZE" in os.environ and "RANK" in os.environ:
        try:
            ...
            print(f"WARNING: torch.distributed initialization failed: {e}. "
                  "Continuing with single-process execution.")
    else:
        # Ensure we select a valid CUDA device in single-process runs
        if torch.cuda.is_available():
    ...
    if args.ckpt_pt:
        ...
        config = ConditionalModelConfig(
            ...
            # using context compression factor 4
            dim=512,
            n_head=8,
            n_layer=8,
            ffn_mult=4,
            ...
            muon_lr=3e-2,
            warmup_steps=1000,
            num_steps=1500, # lr constant
            cooldown_steps=3000, # lr constant
            finetune_muon_lr=1e-2,
            finetune_adam_lr=1e-4,
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda it: get_lr(optimizer, it, config)) 
    ...
    global_step = 0
    config_ft = config
    config_ft.muon_lr = config.finetune_muon_lr
    config_ft.adam_lr  = config.finetune_adam_lr
    optimizer = create_muon_optimizer(model, config_ft)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda it: get_lr(optimizer, it, config_ft)) 
